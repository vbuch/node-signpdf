...
describe('Test signing', () => {
    it('expects PDF to be Buffer', async () => {
        try {
            await signer.sign('non-buffer', Buffer.from(''));
    ...
    it('expects P12 certificate to be Buffer', async () => {
        try {
            await signer.sign(Buffer.from(''), 'non-buffer');
            expect('here').not.toBe('here');
        } catch (e) {
            expect(e instanceof SignPdfError).toBe(true);
            expect(e.type).toBe(SignPdfError.TYPE_INPUT);
    ...
    it('expects a reasonably sized placeholder', async () => {
        try {
            ...
            await signer.sign(pdfBuffer, p12Buffer);
            expect('here').not.toBe('here');
        } catch (e) {
            expect(e instanceof SignPdfError).toBe(true);
            expect(e.type).toBe(SignPdfError.TYPE_INPUT);
    ...
    it('signs input PDF', async () => {
        ...
        pdfBuffer = await signer.sign(pdfBuffer, p12Buffer);
    ...
    it('signs a ready pdf two times', async () => {
        ...
        signedPdfBuffer = await signer.sign(signedPdfBuffer, secondP12Buffer, {passphrase: 'node-signpdf'});
    ...
    it('signs big PDF twice producing big AcroForm ID on the first time', async () => {
        ...
        pdfBuffer = await signer.sign(pdfBuffer, secondP12Buffer, {passphrase: 'node-signpdf'});
        expect(pdfBuffer instanceof Buffer).toBe(true);
