...
import { extractSignature, Signer, SignPdfError } from "@signpdf/utils";
import {
  readTestResource,
  createPdfkitDocument,
} from "@signpdf/internal-utils";
...
describe("Test signing", () => {
  ...
  it("expects a reasonably sized placeholder", async () => {
    try {
      ...
      expect(e.type).toBe(SignPdfError.TYPE_INPUT);
      expect(e.message).toMatchInlineSnapshot(
        '"Signature exceeds placeholder length: 3224 > 4"'
      );
    }
  });
  ...
  it("signs detached", async () => {
    ...
    await signpdf.sign(
      pdfBuffer,
      new P12Signer(readTestResource("certificate.p12"))
    );
  ...
  it.only("Signs the resource from issue 79", async () => {
    let pdfBuffer = readTestResource("issue-79-test.pdf");
    const p12Signer = new P12Signer(readTestResource("certificate.p12"));
  ...
  it("signs a ready pdf for the second time", async () => {
    ...
    const p12Signer = new P12Signer(readTestResource("withpass.p12"), {
  ...
  it("signs big PDF twice producing big AcroForm ID on the first time", async () => {
    ...
    const secondP12Signer = new P12Signer(readTestResource("withpass.p12"), {
      passphrase: "node-signpdf",
    });
    pdfBuffer = plainAddPlaceholder({
      pdfBuffer,
    ...
    const { signature: secondSignature, signedData: secondSignatureData } =
